...
// Controla el asistente paso a paso para dar de alta un vehículo,
// incluyendo validación por pasos, resumen final y autocompletado por matrícula/VIN.
(function () {
        ...
        // Recupera un campo del formulario por su atributo name.
        function getField(name) {
            return form.querySelector('[name="' + name + '"]');
        }

        // Muestra mensajes de estado del autocompletado, diferenciando error o éxito.
        ...
        // Asigna un valor a un input o select y dispara eventos para sincronizar UI/validaciones.
        ...
        // Consulta el endpoint interno de lookup y rellena automáticamente datos del vehículo.
        ...
        // Sincroniza el contenido del editor enriquecido con el textarea real antes de validar/enviar.
        function syncEditor() {
            if (!descriptionField) {
                return;
            }
        ...
        // Devuelve los campos interactivos del paso actual, excluyendo ocultos y deshabilitados.
        ...
        // Valida solo el paso activo del wizard usando las validaciones nativas del navegador.
        ...
        // Prepara el valor de un campo para mostrarlo en el resumen final del asistente.
        function formatFieldValue(field) {
            if (field.type === 'file') {
                ...
                return Array.from(field.files).map(function (file) {
                    return file.name;
                }).join(', ');
            }
            if (field.tagName === 'SELECT') {
        ...
        // Construye un resumen visual con todos los datos introducidos antes de guardar.
        ...
        // Muestra el paso actual, actualiza el progreso y alterna botones de navegación/envío.
        function renderStep() {
...
/* ===== base.html :: script 1 ===== */
// Inserta en el <head> los datos estructurados JSON-LD serializados desde Django.
(function () {
...
/* ===== base.html :: script 2 ===== */
// Gestiona el menú responsive de navegación en móviles.
(function () {
            ...
            // Abre el menú y actualiza atributos de accesibilidad.
            function openMenu() {
            ...
            // Cierra el menú y restaura el estado visual del documento.
            function closeMenu() {
        ...
        // Controla el cierre manual y automático de los mensajes flash.
        (function () {
            ...
            // Oculta un aviso con pequeña animación antes de eliminarlo del DOM.
...
// Gestiona el modal de mensajes, marcado como leído y actualización de contadores del panel admin.
(function () {
    // Abre el modal con el contenido del mensaje y lo marca como leído si procede.
    async function openMessage(id, name, email, phone, subject, message, isUnread) {
        ...
        if (!modal || !modalSubject || !modalFrom || !modalEmail || !modalPhone || !modalMessage) {
            return;
        }

        modalSubject.textContent = subject;
        modalFrom.textContent = 'De: ' + name;
        modalEmail.textContent = email;
        modalPhone.textContent = phone || 'No proporcionado';
        modalMessage.textContent = message;
        modal.classList.add('show');

        if (isUnread) {
            const markedAsRead = await markMessageAsRead(id);
            if (markedAsRead) {
    ...
    // Envía una petición AJAX para marcar un mensaje como leído.
    ...
    // Refleja visualmente en la tabla que el mensaje ya fue leído.
    ...
    // Recalcula y actualiza los badges de mensajes pendientes en panel y navegación.
    function updateUnreadCounters() {
        ...
        const navBadge = document.querySelector('a[href="/mensajes-contacto/"] .nav-badge');
        if (navBadge) {
    ...
    // Recupera el token CSRF desde las cookies para peticiones POST fetch.
...
// Gestiona la ordenación manual y drag & drop de la galería en la edición de stock.
(function () {
    ...
    (function () {
        ...
        // Devuelve todas las tarjetas de imagen actualmente visibles en la galería.
        function getItems() {
            return Array.from(list.querySelectorAll('.gallery-item'));
        }

        // Sincroniza el orden visual con los inputs numéricos que se enviarán al backend.
        ...
        // Habilita o deshabilita botones subir/bajar según posición de cada imagen.
        ...
        // Limpia estilos temporales usados durante el arrastre de imágenes.
...
// Gestiona la selección de favoritos para compararlos y activa/desactiva el botón de comparar.
(function () {
        ...
        // Obtiene los ids seleccionados mediante checkboxes en la lista de favoritos.
        ...
        // Actualiza el texto informativo y el enlace del comparador según la selección actual.
...
/* ===== index.html :: script 1 ===== */
// Controla dos experiencias de la home: el hero rotatorio y el carrusel/slider de reseñas.
(function () {
		...
		if (hero && featured.length > 1) {
            ...
            // Pinta en el hero el vehículo destacado correspondiente al índice activo.
            function paintHero(item) {
				...
				if (heroHighlight) {
                    ...
                    heroHighlight.append(document.createTextNode('Destacado: '));

                    const link = document.createElement('a');
                    link.className = 'hero-highlight-link';
                    link.href = item.url || '#';
                    link.textContent = item.title || '';
                    heroHighlight.appendChild(link);
                    heroHighlight.append(document.createTextNode(' · ' + (item.price || '')));
				}
            ...
            // Cambia el vehículo visible en el hero y sincroniza indicadores.
            function showByIndex(newIndex) {
				index = (newIndex + featured.length) % featured.length;
				paintHero(featured[index]);
			}

            // Activa la rotación automática del hero destacado.
            ...
            // Detiene la rotación automática para evitar conflictos con interacción del usuario.
		...
		if (reviewsTrack && reviewCards.length > 0) {
            ...
            // Calcula el ancho de desplazamiento necesario para avanzar entre grupos de reseñas.
            ...
            // Estima cuántas tarjetas de reseña caben visibles según el ancho actual.
            ...
            // Recalcula las páginas lógicas del slider de reseñas para móvil/tablet/escritorio.
            ...
            // Dibuja los dots de navegación del slider de reseñas.
            ...
            // Determina qué página del slider de reseñas está actualmente visible.
            ...
            // Devuelve el índice inicial de la página actualmente visible.
            function getCurrentReviewPageStart() {
                return reviewPages[getActiveReviewPage()] || 0;
            }

            // Sincroniza el dot activo con la posición actual del carrusel.
            ...
            // Desplaza el track de reseñas a la página solicitada.
            ...
            // Detiene la reproducción automática del slider de reseñas.
            ...
            // Inicia la reproducción automática de reseñas cuando procede.
            function startReviewsAutoplay() {
				...
				reviewsAutoplayTimer = setInterval(function () {
                    const nextPage = (getActiveReviewPage() + 1) % reviewPages.length;
                    goToReview(reviewPages[nextPage]);
				}, 4200);
			}
			...
			window.addEventListener('resize', function () {
                const currentPageStart = getCurrentReviewPageStart();
				buildReviewPages();
				renderReviewDots();
                goToReview(currentPageStart);
			});
...
// Gestiona la galería de detalle, los slots de prueba de conducción y la calculadora de financiación.
(function () {
    ...
    (function () {
        ...
        // Rellena el selector de horas disponibles para la prueba de conducción.
        ...
        // Consulta al backend las horas disponibles para la fecha elegida.
    ...
    (function () {
        ...
        // Calcula la cuota mensual estimada de financiación usando amortización estándar.
        ...
        // Recalcula y pinta en pantalla el resultado de la calculadora financiera.
        function render() {
...
/* ===== stock.html :: script 1 ===== */
// Gestiona la apertura de filtros avanzados y la selección de vehículos para comparar desde inventario.
(function () {
        ...
        let isExpandedInStorage = false;

        try {
            isExpandedInStorage = window.localStorage.getItem(storageKey) === 'true';
        } catch (error) {
            isExpandedInStorage = false;
        }

        // Abre o cierra el panel de filtros avanzados actualizando accesibilidad y clases CSS.
        function setFiltersState(expanded) {
            ...
            try {
                ...
                // Si el almacenamiento no está disponible, el filtro sigue funcionando sin persistencia.
            }
        }
    ...
    (function () {
        ...
        // Devuelve los ids de los vehículos marcados para el comparador.
        function getSelectedIds() {
            return checkboxes
                .filter(function (cb) { return cb.checked; })
                .map(function (cb) { return cb.value; });
        ...
        // Habilita o bloquea el comparador y actualiza su enlace según el número seleccionado.
        function renderCompareState() {
            const selected = getSelectedIds();
            const count = selected.length;
